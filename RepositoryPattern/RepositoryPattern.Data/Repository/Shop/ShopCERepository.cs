...
{
    public sealed class ShopCERepository<T> : IGenericRepository<T> where T : class
    {
          #region Fields

        private readonly ShopCompactContext m_Context;
        ...
        public ShopCERepository(ShopCompactContext context)
        {
            m_Context = context;
        ...
        {
            if (m_Context.Entry(entity).State == EntityState.Detached) 
            {
                m_DbSet.Attach(entity);
            }
